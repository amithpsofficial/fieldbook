<x-app-layout>
    <x-slot name="header">Add New Land</x-slot>

    <div class="max-w-xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-base font-semibold text-gray-800 mb-5">Land Details</h3>

            <form action="{{ route('lands.store') }}" method="POST" class="space-y-4">
                @csrf

                <div>
                    <x-input-label for="name" value="Land Name *" />
                    <x-text-input id="name" name="name" type="text" class="mt-1" :value="old('name')" placeholder="e.g. Flat Land, Near Stream Land" />
                    <x-input-error :messages="$errors->get('name')" class="mt-1" />
                </div>

                <div x-data="{ year: {{ old('plantation_year', date('Y')) }} }">
                ...
                <div>
                    ...
                    <x-text-input id="location" name="location" type="text" class="mt-1" :value="old('location')" placeholder="e.g. Thrissur, Kerala" />
                ...
                <div class="flex items-center justify-between pt-3">
                    ...
                    <x-primary-button>Save Land</x-primary-button>
                </div>
            </form>
        </div>
    </div>
